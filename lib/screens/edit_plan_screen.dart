...
class _EditPlanScreenState extends State<EditPlanScreen> {
  ...
  void _duplicateSet(int exerciseIndex, int setIndex) {
    setState(() {
      ...
      final setToCopy = exercise.setDefaults[setIndex];
      final newSets = List<gym.Set>.from(exercise.setDefaults);
      newSets.insert(
        setIndex + 1,
        gym.Set(
          reps: setToCopy.reps,
      ...
      _exercises[exerciseIndex] = ExerciseTemplate(
        id: exercise.id,
        name: exercise.name,
        orderIndex: exercise.orderIndex,
        setDefaults: newSets,
  ...
  void _updateSet(int exerciseIndex, int setIndex, int reps, double weight) {
    setState(() {
      final exercise = _exercises[exerciseIndex];
      final currentSet = exercise.setDefaults[setIndex];
      final newSets = List<gym.Set>.from(exercise.setDefaults);
      newSets[setIndex] = gym.Set(
        reps: reps,
        weight: weight,
        rpe: currentSet.rpe,
  ...
  void _deleteSet(int exerciseIndex, int setIndex) {
    setState(() {
      final exercise = _exercises[exerciseIndex];
      if (exercise.setDefaults.length <= 1) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Widget _buildExerciseCard(
      ExerciseTemplate exercise, int exerciseIndex, Color accent) {
    return Padding(
      padding: const EdgeInsets.all(12),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              ...
              Text(
                '${exercise.setDefaults.length} SETS',
                style: GoogleFonts.jetBrainsMono(
                    fontSize: 10, color: textSecondaryColor(context)),
              ),
              const SizedBox(width: 8),
          ...
          const SizedBox(height: 12),
          Divider(color: borderColor(context)),
          ...exercise.setDefaults.asMap().entries.map((entry) {
            final setIndex = entry.key;
          ...
          InkWell(
            onTap: () {
              setState(() {
                final existing = _exercises[exerciseIndex];
                final newSets = List<gym.Set>.from(existing.setDefaults)
                  ..add(gym.Set(reps: 8, weight: 0, rpe: null));
                _exercises[exerciseIndex] = ExerciseTemplate(
                  id: existing.id,
                  name: existing.name,
