...
class WorkoutSessionRepository {
  ...
  Future<void> deleteSession(int index) async {
    ...
    // 3. Sync to API in background - don't await
    // Note: Delete sync would need session ID
    final isOnline = await _connectivityService.isOnline();
    if (!isOnline) {
      // Queue delete operation if we had session IDs
      // await _syncQueueService.addSessionDelete(sessionId);
    }
  }
    ...
    try {
      ...
      if (operation == 'create') {
        ...
        if (response.statusCode != 201) {
          print(
              'Failed to sync session to API: ${response.statusCode} ${response.body}');
        }
      } else if (operation == 'update' && session.id != null) {
        final response = await _apiService.put('/sessions/${session.id}', body);
        if (response.statusCode != 200) {
          print(
              'Failed to sync session update to API: ${response.statusCode} ${response.body}');
        }
      }
    } catch (e) {
      print('Error syncing session to API: $e');
    }
  }
  ...
  Future<void> deleteSessionForPlanAndWeek(
    ...
    // 3. Sync to API in background - don't await
    // Note: Would need to track which sessions to delete by ID
    final isOnline = await _connectivityService.isOnline();
    if (!isOnline) {
      // Queue delete operations if we had session IDs
      // await _syncQueueService.addSessionDelete(sessionId);
    }
  }
